...
class RecordingSettingsMenuR7Test {
    ...
    fun `the Settings menu exposes switch and manual configuration with watch sized actions`() {
        assertTrue(menu.contains("Dialog(onDismissRequest = onDismiss, properties = DialogProperties(usePlatformDefaultWidth = false))"))
        assertTrue(menu.contains("switchEntryLabelRes(ui.switchSearching, ui.targets.isNotEmpty())"))
        assertTrue(menu.contains("onClick = onSwitch"))
        assertTrue(menu.contains("R.string.screen_config_title"))
        assertTrue(menu.contains("onClick = onConnectionSettings"))
        assertTrue(menu.contains("SettingsMenuAction("))
        assertTrue(menu.contains("modifier.height(52.dp)"))
        assertTrue(menu.contains(".clickable(enabled = enabled, onClick = onClick)"))
        assertFalse(menu.contains("R.string.action_cancel"))

        val strings = File("src/main/res/values/strings.xml").readText()
        assertTrue(strings.contains("name=\"ready_open_config\">设置<"))
        assertTrue(strings.contains("name=\"settings_section_title\">电脑连接<"))
