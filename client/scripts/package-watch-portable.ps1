...
param(
    ...
    # artefact instead of on a copy of the template inside this script.
    [switch]$NotesOnly
)
...
# ── The shipped notes: ONE template, checked on the REAL generated bytes ──────
#
# The notes are written from a SINGLE-QUOTED here-string so nothing in them is interpreted: a
# backtick inside a double-quoted here-string is an escape character, and an earlier version
# silently shipped a form-feed where "frontendDist" should have been.
...
<#
  Writes the two shipped notes into $Directory.
  This is the ONLY place the shipped wording exists, so -NotesOnly (used by the unit tests) and a
  real package can never drift apart.
#>
function New-PortableNotes {
    param(
    ...
    # BUILD-INFO is assembled line by line (a here-string inside a function would have to escape
    # its own indentation). ASCII only, so PowerShell 5.1's encoding handling cannot change it.
    $versionLines = @(
        "package=$packageBase",
        'entry=SayIt.exe',
        'build=debug',
    ...
    [System.IO.File]::WriteAllText(
        (Join-Path $Directory 'BUILD-INFO.txt'),
...
<#
  The static gate on the GENERATED notes: plain text, correct paths, no control characters and no
  pointer at a settings entry that does not exist. It reads the artefacts, never the template, so
  it cannot pass on wording that was never shipped.
#>
function Test-PortableNotes {
    ...
    foreach ($noteName in @('README-PORTABLE.txt', 'BUILD-INFO.txt')) {
        ...
        if (-not (Test-Path $notePath)) { Fail "$noteName was not generated" }
    ...
    $readmeText = [System.IO.File]::ReadAllText((Join-Path $Directory 'README-PORTABLE.txt'), [System.Text.Encoding]::UTF8)
...
# template inside this script. This mode writes them into -OutputDir, runs the SAME static gate the
# full package run uses, prints the paths and exits without building, copying or zipping anything.
# It deliberately runs BEFORE the clean-tree gate: generating notes cannot ship a binary, and a test
# has to be able to run it from a worktree that is still being edited.
if ($NotesOnly) {
