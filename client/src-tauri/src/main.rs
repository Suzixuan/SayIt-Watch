...
//
// 注意：绝不能再加 --ignore-certificate-errors。它会全局关闭 WebView2 的 TLS 校验，
// 使中间人可冒充后端（公告/反馈/更新清单/WebSocket 全部走 WebView 网络栈）。
// 本地后端默认走 http://127.0.0.1，不存在自签证书问题；有自签证书需求应改用受信证书池。
...
fn main() {
    ...
    // 不读取环境变量覆盖浏览器参数：默认参数恒定，防止启动环境注入
    // （如 WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS 里被塞进 --ignore-certificate-errors）。
    std::env::set_var("WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS", WEBVIEW2_BROWSER_ARGS);
    log::info!(
        "WebView2 environment userDataDir={:?} argsLen={} argsFingerprint={}",
        dirs::data_local_dir()
            .unwrap_or_else(|| std::path::PathBuf::from("."))
            .join("com.sayit.app")
            .join("EBWebView"),
        WEBVIEW2_BROWSER_ARGS.len(),
        browser_args_fingerprint(WEBVIEW2_BROWSER_ARGS),
    );
