...
电脑和手表连上同一个可信 Wi-Fi、启动 Windows 接收端后，手表会查找电脑并验证连接。第一次仍需在手表上填入本机生成的 Token；之后电脑的局域网 IP 变了，不必每次手动改地址。电脑比手表晚启动时，手表保持在前台也会继续尝试连接。

如果要换到另一台电脑，从「设置 → 切换电脑」查看并选择；返回会保留当前电脑。发现失败时，再到「连接设置」手动填写 IP 和端口。目前单电脑自动发现已在 Galaxy Watch 7 上验证；台式机与笔记本同时在线时的切换和换机后录音还在验收。
