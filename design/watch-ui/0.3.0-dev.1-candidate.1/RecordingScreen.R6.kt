...
private fun ReadyScreen(viewModel: RecordingViewModel, ui: WatchUiState, discovery: DiscoveryState, canRecord: Boolean, hasPermission: Boolean, onRequestPermission: () -> Unit) {
    WatchDial {
        if (!hasPermission) {
            ...
            // 1C-D-04@R5: a single explicit column instead of hand-placed offsets.
            // The previous offset layout put the switch entry at y=+106 and the
            // settings entry at y=+88, so they overlapped on the round screen; it also
            // rendered the switch entry only when more than one computer was known,
            // which hid the one entry point the user actually needs.
            Column(
                ...
                Spacer(Modifier.height(8.dp))
                // 1C-D-04@R5 必修 1: the search/switch entry is ALWAYS present on Ready
                // — 0, 1 or ≥2 authenticated computers — so that a user whose single
                // saved computer is online can still actively look for the other one.
                // It runs the production `requestSwitch()` (one bounded browse); the
                // manual IP screen keeps its own separate entry below.
                PillAction(
                    label = stringResource(
                        switchEntryLabelRes(ui.switchSearching, ui.targets.isNotEmpty()),
                    ),
                ...
                Spacer(Modifier.height(6.dp))
                // Dim, low-key settings entry (needed for first-time config and for
                // the bounded manual fallback). Never opens itself.
                Text(
                    stringResource(R.string.ready_open_config),
                    fontSize = 9.sp,
                    color = DialMuted,
                    modifier = Modifier.heightIn(min = WatchUiMetrics.RowMinHeightDp)
                        .clickable(enabled = !ui.isUploading) { viewModel.openConfig() },
                )
            }
        }
        if (ui.switchOpen) {
...
/**
 * 1C-D-04@R5 必修 1 — is the Ready screen's search/switch entry available?
 *
 * A pure decision so the invariant is JVM-testable without Compose rendering: the
 * entry must exist for **every** Ready state, including the normal "one saved
 * computer is online" case that the real Galaxy Watch run demonstrated. R4 gated it
 * on `targets.size > 1`, which made the user's most common switch path unreachable.
 */
